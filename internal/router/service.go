...
func (s *Service) analyzeByModel(ctx context.Context, text string) (Decision, error) {
	...
	systemPrompt := "你是 EDR 意图路由器。请把用户输入路由成结构化 JSON。\n" +
		"可选 action 只有：none, hosts, incidents, detections, logs, isolate, release, iocs, tasks, task_result, send_instruction, virus_by_host, virus_by_hash, virus_hash_hosts, virus_scan_record, ioa, ioa_network, strategy, host_offline, plans, plan_add, plan_edit, plan_cancel, instruction_policy_list, instruction_policy_update, instruction_policy_save_status, instruction_policy_delete, instruction_policy_sort, instruction_policy_add, remove_host。\n" +
		...
		"\"action\":\"none|hosts|incidents|detections|logs|isolate|release|iocs|tasks|task_result|send_instruction|virus_by_host|virus_by_hash|virus_hash_hosts|virus_scan_record|ioa|ioa_network|strategy|host_offline|plans|plan_add|plan_edit|plan_cancel|instruction_policy_list|instruction_policy_update|instruction_policy_save_status|instruction_policy_delete|instruction_policy_sort|instruction_policy_add|remove_host\"," +
		"\"confidence\":0.0," +
		"\"hostname\":\"\"," +
		"\"client_id\":\"\"," +
		"\"client_ip\":\"\"," +

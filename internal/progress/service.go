...
func (s *Service) progressPrompt() string {
	base := "你是 AI 助手的进度播报员。请把内部操作步骤改写成发给终端用户的一句中文进度说明。要求：第一人称、18 到 40 字、友好自然、优先反映新的进展、避免重复同义句、不要输出编号、不要泄露路径/密钥/API 细节、不要夸大结果、不要使用 markdown。可以偶尔带 1 个轻量 emoji（如🙂、🔎），但不要每句都带。只输出一句话。"
	if s.prompt != nil {
...
func (s *Service) toolResultPrompt() string {
	base := "你是 AI 助手的进度播报员。现在某个工具已经返回了真实结果，请把最值得告诉终端用户的中间结论改写成一句中文进度说明。要求：第一人称、20 到 50 字、友好自然、只基于给定真实结果、不编造没有出现的字段、不输出编号或 markdown、不泄露路径/密钥/API 细节。只输出一句话。"
	if s.prompt != nil {

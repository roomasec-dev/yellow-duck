...
func plannerPrompt(skillsPrompt string, memoryText string, latestArtifact protocol.Artifact) string {
	base := "你是驻留在 IM 里的安全分析员兼工具规划器。你的目标不是把工具用完，而是像人类分析员一样判断用户真正要什么、查到什么程度够了、什么时候该收口，并输出 JSON。\n" +
		...
		"20.5 如果用户在查看自动响应策略列表，优先规划 edr_instruction_policy_list；如用户明确要新增、更新、删除、排序、启停，则规划对应 edr_instruction_policy_add / update / delete / sort / save_status。\n" +
		"20.6 如果用户在查病毒查杀记录或按主机/哈希扩线，分别优先规划 edr_virus_scan_record / edr_virus_by_host / edr_virus_by_hash / edr_virus_hash_hosts。\n" +
